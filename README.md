...
Print mode's flags drift between releases, so the skill tells Claude to trust `agy --help` and `agy models` over its own tables when they disagree.
...
Refresh the marketplace catalogue later with `/plugin marketplace update agent-tools`, then `/plugin update antigravity@agent-tools` and `/reload-plugins`.
...
Then remove it at that scope and install the replacement there, substituting `project` or `local` where the example says `user`:
...
From inside a session the same steps are `/plugin uninstall gemini`, `/plugin install antigravity@agent-tools`, then `/reload-plugins`; the CLI form above is the one that takes an explicit scope.
...
For artifacts that evolve across revisions (specs, plans, designs), the skill runs a convergence loop: review, fix, re-review, until the reviewer gives an affirmative verdict, scope drift appears, or you stop. Rounds resume a pinned conversation ID when one can be captured, and fall back to a stateless round with a prior-findings block when none can. Each round re-supplies the current artifact, so it never critiques a stale version.
